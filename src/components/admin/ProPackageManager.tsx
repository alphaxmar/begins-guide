...
const ProPackageManager = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (selectedPackage) {
      updatePackage.mutate({ 
        ...
        onSuccess: () => {
          toast.success('อัปเดตแพ็กเกจ PRO สำเร็จ');
      ...
      createPackage.mutate(packageData, {
        onSuccess: () => {
          toast.success('สร้างแพ็กเกจ PRO สำเร็จ');
          setIsDialogOpen(false);
          setSelectedPackage(null);
        },
        onError: () => {
  ...
  const handleDelete = (id: string) => {
    if (confirm('คุณแน่ใจหรือไม่ที่จะลบแพ็กเกจนี้?')) {
      deletePackage.mutate(id, {
        onSuccess: () => {
          toast.success('ลบแพ็กเกจ PRO สำเร็จ');
        },
        onError: () => {
  ...
  const openEditDialog = (pkg: ProPackage) => {
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold flex items-center gap-2">
            ...
            จัดการแพ็กเกจ Begins.guide PRO
          </h2>
          <p className="text-muted-foreground mt-1">
            สร้าง แก้ไข และจัดการแพ็กเกจ PRO สำหรับลูกค้า (รองรับ Subscription)
          </p>
        </div>
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                ...
                {selectedPackage ? 'แก้ไขแพ็กเกจ PRO' : 'เพิ่มแพ็กเกจ PRO ใหม่'}
              </DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  placeholder="เช่น PRO Monthly, PRO Annual"
                  required
                />
              </div>
              ...
              <div>
                ...
                <Textarea
                  ...
                  placeholder="คำอธิบายแพ็กเกจ PRO"
                />
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder="990 (สำหรับรายเดือน) หรือ 9900 (สำหรับรายปี)"
                  required
                />
              </div>
              ...
              <div>
                <Label htmlFor="duration_months">
                  ...
                  <span className="text-sm text-muted-foreground ml-1">
                    - 1 สำหรับรายเดือน, 12 สำหรับรายปี
                  </span>
                </Label>
                <Input
                  id="duration_months"
                  ...
                  placeholder="1 (รายเดือน) หรือ 12 (รายปี)"
                />
              </div>
        ...
        {packages && packages.length > 0 ? packages.map((pkg) => (
              ...
              <div className="space-y-4">
                <div className="flex items-baseline gap-2">
                  ...
                  <Badge variant="outline" className="ml-2">
                    {pkg.duration_months === 1 ? 'รายเดือน' : 
                     pkg.duration_months === 12 ? 'รายปี' : 
          ...
          <Card className="border-2 border-dashed border-gray-300">
            <CardContent className="p-8 text-center">
              ...
              <p className="text-muted-foreground text-lg">ยังไม่มีแพ็กเกจ PRO</p>
              <p className="text-sm text-muted-foreground mt-1">
                คลิก "เพิ่มแพ็กเกจใหม่" เพื่อเริ่มสร้างแพ็กเกจ PRO แรกของคุณ
              </p>
            </CardContent>
          </Card>
        )}
...
export default ProPackageManager;
